...
`ifdef SIM
	...
	reg wrreq_old = 0, rdreq_old = 0;
	always @(posedge clock) begin
		wrreq_old <= wrreq;
		rdreq_old <= rdreq;
	end
	wire wr = wrreq & ~wrreq_old;
	wire rd = rdreq & ~rdreq_old;
	...
	always @(posedge clock) begin
		...
		if (wr && !rd) begin
			if (amount != 12'hFFF) begin
				amount <= amount + 1;
			end
		end else if (!wr && rd) begin
			if (amount != 12'h000) begin
				amount <= amount - 1;
			end
		end
	end
		
	wire [31:0] q = MEM[rd_pos];
...
// Retrieval info: GEN_FILE: TYPE_NORMAL CDFIFO_bb.v FALSE
